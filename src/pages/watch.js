...
export default function WatchPage() {
  ...
  useEffect(() => {
    const fetchVideoData = async () => {
      ...
      try {
        ...
        const [videoData, relatedData] = await Promise.all([videoResponse.json(), relatedResponse.json()]);
  ...
  if (!videoId) return (
    <div className="min-h-screen bg-gradient-to-b from-black to-gray-800 text-white flex items-center justify-center">
      ...
      <main className="container mx-auto px-4 py-6 md:px-6">
        <div className="grid 2xl:grid-cols-3 gap-6">
          <div className="2xl:col-span-2">
            <section className="relative aspect-video w-full overflow-hidden rounded-xl bg-gray-900 p-[2px] group hover:p-[3px] transition-all duration-300 shadow-lg shadow-gray-900">
              <div className="absolute inset-0 bg-gradient-to-r from-orange-500 via-red-500 to-yellow-400 rounded-xl animate-gradient-xy opacity-75"></div>
              <div className="relative w-full h-full bg-gray-900 rounded-xl overflow-hidden z-10">
                {video?.id ? (
                  <VideoPlayer videoId={video.id} onReady={handlePlayerReady} />
                ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              {relatedVideos.slice(0, showCount).map((relatedVideo) => (
                ...
                >
                  ...
                  <div className="relative w-full h-full bg-gray-900/90 rounded-lg overflow-hidden z-10 backdrop-blur-sm">
                    <div className="relative aspect-video">
                      {relatedVideo.thumbnail?.[0]?.url && (
                        <Image
                          src={thumbnailUrl(relatedVideo.videoId) || relatedVideo.thumbnail[0].url}
